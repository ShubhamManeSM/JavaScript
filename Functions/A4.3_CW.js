...
function calculateOvertimePay(hourlyRate,hoursWorked,overtime){
  let overtimePay = 0;
  if(hoursWorked > 40){
...
let taxRate = 0.2;
let insuaranceRate = 0.1;
let retirementRate = 0.05;

function calculateDeductions(grossPay,taxRate,insuaranceRate,retirementRate){
  let taxAmount = grossPay * taxRate;
  let insuaranceAmount = grossPay * insuaranceRate;
  let retirementAmount = grossPay * retirementRate;
  return taxAmount + insuaranceAmount + retirementAmount
}

let deductions = calculateDeductions(grossPay,taxRate,insuaranceRate,retirementRate);
...
console.log(generatePayStub("Shubham",hourlyRate,hoursWorked,overTimePay,grossPay,deductions,netPay,bonusAmount))
